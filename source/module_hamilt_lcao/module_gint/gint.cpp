...
//!!!!!! 
...
#endif
		{
			...
			if(inout->job==Gint_Tools::job_type::dvlocal)
			{
				...
				// HEREADD3
					ModuleBase::Memory::record("#0",sizeof(double) * 3 * nnrg);
            ...
            || inout->job==Gint_Tools::job_type::force_meta)
			{
				if(inout->isforce)
				{
					...
					// HEREADD3
					ModuleBase::Memory::record("#1",sizeof(double) * inout->fvl_dphi->nr * inout->fvl_dphi->nc);
				...
				if(inout->isstress)
				{
					...
					// HEREADD3
					ModuleBase::Memory::record("#2",sizeof(double) * inout->svl_dphi->nr* inout->svl_dphi->nc);
...
//!!!!!!
			...
			for(int grid_index = 0; grid_index < this->nbxx; grid_index++)
			{
				...
				// HEREADD3
					ModuleBase::Memory::record("Gint::cal_gint",sizeof(int)* this->bxyz);
				...
				else if(inout->job == Gint_Tools::job_type::tau)
				{
                    int* vindex = Gint_Tools::get_vindex(this->bxyz, this->bx, this->by, this->bz,
                        this->nplane, this->gridt->start_ind[grid_index], ncyz);
							// HEREADD3
					ModuleBase::Memory::record("#4",sizeof(int)* this->bxyz);
		...
		// HEREADD3
					ModuleBase::Memory::record("#5",sizeof(double)* this->bxyz);
