...
void Tensor::resize(const TensorShape& new_shape) {
    ...
    if (buffer_ && buffer_->GetAllocatedBytes() < new_shape.NumElements() * SizeOfType(data_type_)) {
        ...
        //HEREADD3
        // std::cout << "!!!!!! " << SizeOfType(data_type_) << std::endl;

        // ModuleBase::Memory::record("***TensorBuffer::data_",new_shape.NumElements() * SizeOfType(data_type_));
    }
    shape_ = new_shape;
}
...
// Resize tensor object with the given tensor_shape
void Tensor::resize(const TensorShape& new_shape, const std::string& str) {
    if (shape_ == new_shape) {
        return;
    }
    REQUIRES_OK(buffer_->OwnsMemory() || this->NumElements() == 0,
    ...
    if (buffer_ && buffer_->GetAllocatedBytes() < new_shape.NumElements() * SizeOfType(data_type_)) {
        ...
        ModuleBase::Memory::record(str,new_shape.NumElements() * SizeOfType(data_type_));
    }
    shape_ = new_shape;
}
